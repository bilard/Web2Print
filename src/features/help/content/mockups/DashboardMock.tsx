...
export function DashboardMock() {
  return (
    <div className="w-full max-w-[480px] bg-background border border-white/10 rounded-md overflow-hidden pointer-events-none">
      <div className="grid grid-cols-[120px_1fr]">
        <div className="bg-surface border-r border-white/10 py-2 flex flex-col gap-0.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {projects.map((p, i) => (
              <div key={i} className="rounded border border-white/10 overflow-hidden">
                ...
                <div className="px-2 py-1 text-[10px] text-white/70 truncate bg-surface">{p.name}</div>
              </div>
            ))}
          </div>
        </div>
