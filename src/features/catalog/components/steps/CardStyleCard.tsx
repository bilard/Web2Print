// src/features/catalog/components/steps/CardStyleCard.tsx
// Carte « Style des fiches » : réglages COSMÉTIQUES bornés (couleurs des objets,
// échelles de texte, rayon, visibilité) appliqués par variables CSS par-dessus le
// template fluide — impossible de casser la mise en page. Aperçu live à droite.
import { Brush, RotateCcw } from 'lucide-react'
import type { PromoFields } from '@/features/retail-promo/promoTypes'
import { DEFAULT_CARD_STYLE, type CatalogCardStyle, type CatalogPlan } from '../../catalogTypes'
import { CardStylePreview } from './CardStylePreview'

interface CardStyleCardProps {
  plan: CatalogPlan
  setPlan: (plan: CatalogPlan) => void
  sampleFields?: PromoFields | null
}

const SCALES: { key: 'nameScale' | 'descScale' | 'priceScale'; label: string }[] = [
  { key: 'nameScale', label: 'Nom' },
  { key: 'descScale', label: 'Description' },
  { key: 'priceScale', label: 'Prix' },
]

const VISIBILITY: { key: keyof Pick<CatalogCardStyle, 'showPromo' | 'showSticker' | 'showKicker' | 'showDesc' | 'showRef' | 'showUnit'>; label: string }[] = [
  { key: 'showPromo', label: 'Cartouche promo' },
  { key: 'showSticker', label: 'Sticker remise' },
  { key: 'showKicker', label: 'Sous-famille' },
  { key: 'showDesc', label: 'Description' },
  { key: 'showRef', label: 'Référence' },
  { key: 'showUnit', label: 'Unité' },
]

export function CardStyleCard({ plan, setPlan, sampleFields }: CardStyleCardProps) {
  const style: CatalogCardStyle = { ...DEFAULT_CARD_STYLE, ...plan.cardStyle }
  const patch = (p: Partial<CatalogCardStyle>) => setPlan({ ...plan, cardStyle: { ...style, ...p } })
  // Couleurs effectives par défaut (héritées du thème) pour initialiser les pickers.
  const COLORS: { key: keyof Pick<CatalogCardStyle, 'promoBg' | 'stickerBg' | 'priceBg' | 'wasBg' | 'kickerBg' | 'nameColor'>; label: string; fallback: string }[] = [
    { key: 'promoBg', label: 'Cartouche', fallback: plan.theme.accent },
    { key: 'stickerBg', label: 'Sticker', fallback: plan.theme.accent },
    { key: 'priceBg', label: 'Prix', fallback: plan.theme.accent },
    { key: 'wasBg', label: 'Prix barré', fallback: plan.theme.headerBg },
    { key: 'kickerBg', label: 'Sous-famille', fallback: plan.theme.headerBg },
    { key: 'nameColor', label: 'Nom', fallback: plan.theme.ink },
  ]

  return (
    <section className="rounded-lg border border-border bg-surface p-4 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <h3 className="flex items-center gap-2 text-sm font-semibold text-white">
          <Brush className="w-4 h-4 text-indigo-400" /> Style des fiches
          <span className="text-xs font-normal text-muted-foreground">— cosmétique seulement, la mise en page fluide est préservée</span>
        </h3>
        <button type="button" onClick={() => setPlan({ ...plan, cardStyle: { ...DEFAULT_CARD_STYLE } })}
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs text-muted-foreground hover:text-white hover:bg-surface-2"
          title="Revenir au style par défaut">
          <RotateCcw className="w-3.5 h-3.5" /> Réinitialiser
        </button>
      </div>

      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] items-start">
        <div className="space-y-4 min-w-0">
          <div>
            <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Taille des textes</div>
            <div className="grid grid-cols-3 gap-4">
              {SCALES.map(({ key, label }) => (
                <label key={key} className="text-xs text-muted-foreground space-y-1">
                  <span className="flex justify-between">{label}<b className="text-white tabular-nums">×{style[key].toFixed(2)}</b></span>
                  <input type="range" min={0.7} max={1.5} step={0.05} value={style[key]}
                    onChange={(e) => patch({ [key]: Number(e.target.value) } as Partial<CatalogCardStyle>)}
                    className="w-full accent-indigo-600" />
                </label>
              ))}
            </div>
          </div>

          <div>
            <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Couleurs des objets</div>
            <div className="flex flex-wrap gap-3">
              {COLORS.map(({ key, label, fallback }) => (
                <label key={key} className="flex flex-col items-center gap-1 text-xs text-muted-foreground">
                  {label}
                  <input type="color" value={style[key] || fallback}
                    onChange={(e) => patch({ [key]: e.target.value } as Partial<CatalogCardStyle>)}
                    className="w-10 h-8 rounded-md bg-surface-2 cursor-pointer" />
                </label>
              ))}
              <label className="flex flex-col items-center gap-1 text-xs text-muted-foreground">
                Arrondi
                <span className="flex items-center gap-2">
                  <input type="range" min={0} max={16} step={1} value={style.radius}
                    onChange={(e) => patch({ radius: Number(e.target.value) })} className="w-20 accent-indigo-600" />
                  <b className="text-white tabular-nums">{style.radius}px</b>
                </span>
              </label>
            </div>
          </div>

          <div>
            <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Éléments affichés</div>
            <div className="flex flex-wrap gap-x-4 gap-y-2">
              {VISIBILITY.map(({ key, label }) => (
                <label key={key} className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer select-none">
                  <input type="checkbox" checked={style[key]} onChange={(e) => patch({ [key]: e.target.checked } as Partial<CatalogCardStyle>)}
                    className="accent-indigo-600" />
                  {label}
                </label>
              ))}
            </div>
          </div>
        </div>

        <CardStylePreview theme={plan.theme} cardStyle={style} fields={sampleFields} />
      </div>
    </section>
  )
}
