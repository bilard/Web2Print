...
const numCls = 'bg-well text-white text-sm rounded px-2 py-1 w-20 border border-white/10 focus:outline-none focus:border-white/25'
...
function Weight({ n, max, muted }: { n: number | undefined; max: number; muted?: boolean }) {
  if (n == null) return <span className="text-[11px] text-white/25 tabular-nums w-24 text-right shrink-0">—</span>
  const pct = max > 0 ? Math.max(2, Math.round((n / max) * 100)) : 0
  return (
    <span className="flex items-center gap-2 w-24 shrink-0 justify-end">
      <span className="h-1.5 rounded-sm bg-[#6366f1]/60" style={{ width: `${pct}%`, maxWidth: '56px' }} />
...
function Branch(
  ...
  return (
    <div className={`flex items-start gap-3 pl-4 border-l border-white/10 py-1.5 ${dimmed ? 'opacity-45' : ''}`}>
      {rank && <span className="text-[11px] text-white/30 tabular-nums w-4 shrink-0 pt-0.5">{rank}</span>}
      <span className="flex-1 min-w-0">
        <span className="text-sm text-white/85 block leading-snug">{label}</span>
        {hint && <span className="text-[11px] text-white/40 block leading-snug">{hint}</span>}
      </span>
      {control}
      {weight}
    </div>
...
function Stage(
  { step, title, subtitle, children, total }:
  { step: string; title: string; subtitle: string; children: React.ReactNode; total?: React.ReactNode },
) {
  return (
    <section className="bg-surface rounded-lg p-4">
      ...
      <p className="text-[11px] text-white/40 mb-3 leading-snug">{subtitle}</p>
...
export function RulesTree(
  ...
  return (
    <div className="space-y-1">
      ...
      <Stage
        step="①" title={t('pw.rules.tree.keys.title')} subtitle={t('pw.rules.tree.keys.sub')}
      ...
      >
        {KEY_BRANCHES.map(({ branch, rank }) => (
          <Branch
            ...
            control={branch === 'origin' ? (
              <input
                type="checkbox" checked={rules.useOriginRefs} className="mt-1 accent-[#6366f1]"
        ...
        <div className="pl-4 border-l border-white/10 pt-2 mt-1 flex flex-wrap gap-4">
          <label className="space-y-1">
            <span className="text-[11px] text-white/60 block">{t('node.pairing-rules.minRefLen.label')}</span>
            <input type="number" min={2} value={rules.minRefLen} className={numCls}
              onChange={(e) => set({ minRefLen: Number(e.target.value) })} />
          </label>
          <label className="space-y-1">
            <span className="text-[11px] text-white/60 block">{t('node.pairing-rules.weakRefLen.label')}</span>
      ...
      <Stage
        step="②" title={t('pw.rules.tree.evidence.title')} subtitle={t('pw.rules.tree.evidence.sub')}
      ...
      >
        {MATCH_EVIDENCES.map((e: MatchEvidence, i) => (
          <Branch
            ...
            control={(
              <input
                type="checkbox" checked={rules.evidence[e]} disabled={e === 'gtin13'} className="mt-1 accent-[#6366f1]"
      ...
      <Stage
        step="③" title={t('pw.rules.tree.vetoes.title')} subtitle={t('pw.rules.tree.vetoes.sub')}
      ...
      >
          ...
          return (
            <Branch
              ...
              control={v === 'price-abyss' ? (
                ...
                <input
                  type="checkbox" checked={on} className="mt-1 accent-[#6366f1]"
                  onChange={(e) => (v === 'family'
        ...
        <Branch label={t('pw.rules.tree.veto.barcodeExempt')} dimmed
          control={<Ban className="w-3 h-3 text-white/20 mt-1" />} />

        <div className="pl-4 border-l border-white/10 pt-2 mt-1 space-y-2">
          <label className="block space-y-1">
            <span className="text-[11px] text-white/60 block">{t('node.pairing-rules.extraFamilies.label')}</span>
            <textarea
              ...
              rows={3} spellCheck={false} placeholder={t('pw.rules.familiesPlaceholder')}
            ...
            />
            <span className="text-[11px] text-white/40 block leading-snug">{t('node.pairing-rules.extraFamilies.help')}</span>
          </label>
          <label className="flex gap-3 items-start cursor-pointer">
            <input type="checkbox" checked={rules.unifyDirectedVetoes} className="mt-1 accent-[#6366f1]"
              onChange={(e) => set({ unifyDirectedVetoes: e.target.checked })} />
            <span>
              <span className="text-sm text-white/85 block">{t('node.pairing-rules.unify.label')}</span>
              <span className="text-[11px] text-white/40 block leading-snug">{t('node.pairing-rules.unify.help')}</span>
            </span>
          </label>
        </div>
      </Stage>
      ...
      <Stage step="④" title={t('pw.rules.tree.price.title')} subtitle={t('pw.rules.tree.price.sub')}>
        <div className="pl-4 border-l border-white/10 flex flex-wrap gap-4 pt-1">
          {([
            ...
            <label key={field} className="space-y-1">
              <span className="text-[11px] text-white/60 block">{t(key)}</span>
