...
import {
  DEFAULT_PAIRING_RULES, MATCH_EVIDENCES, normalizeFamilyLexicon, resolvePairingRules,
...
/** Rend un lexique sous la forme éditable de la textarea. */
export function formatFamilyLexicon(lex: FamilyLexicon): string {
  return Object.keys(lex).sort().map((f) => `${f}: ${lex[f].join(', ')}`).join('\n')
}
...
/** Règles → config. Sert à initialiser le node depuis les défauts, et à recharger dans
 *  l'écran ce qu'un run a écrit. */
export function rulesToConfig(rules: PairingRules): Omit<PairingRulesConfig, 'watchId'> {
  return {
    useOriginRefs: rules.useOriginRefs,
    minRefLen: rules.minRefLen,
    weakRefLen: rules.weakRefLen,
    evidence: MATCH_EVIDENCES.filter((e) => rules.evidence[e]).join(','),
    familyVeto: rules.familyVeto,
    corroborateNumericKeys: rules.corroborateNumericKeys,
    unifyDirectedVetoes: rules.unifyDirectedVetoes,
    priceAbyssRatio: rules.priceAbyssRatio,
    extraFamilies: formatFamilyLexicon(rules.extraFamilies),
    alignedPct: rules.alignedPct,
    minPriceEur: rules.minPriceEur,
    maxDropPct: rules.maxDropPct,
  }
}

/** Config par défaut du node = les règles par défaut. Un node posé et jamais touché ne
 *  change donc RIEN au comportement, ce qui est tout l'enjeu. */
export const DEFAULT_RULES_CONFIG: PairingRulesConfig = {
  watchId: '',
  ...rulesToConfig(DEFAULT_PAIRING_RULES),
}
