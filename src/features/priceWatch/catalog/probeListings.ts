...
export function childListings(html: string, parentUrl: string, max = 40): string[] {
  ...
  for (const u of internalLinks(html, base.hostname, parentUrl)) {
    ...
    // (régression attrapée par les tests de swap-europe).
    const drift = parentSegs.filter((seg, i) => seg.toLowerCase() !== segs[i].toLowerCase())
...
export interface ProbeOptions {
   ...
   * jamais appelé, et la seule descente existante — qui part des pages confirmées — ne
   * démarre pas. C'est exactement le cas de granit-parts.fr : 24 candidats, 0 confirmé,
   * 0 enfant, alors que les grilles sont un niveau plus bas. Bornée par `maxProbes`
   * comme le reste : la file s'allonge, le budget de fetchs ne bouge pas.
   */
