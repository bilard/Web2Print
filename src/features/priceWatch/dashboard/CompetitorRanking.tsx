...
const HEAD_COLS: { Icon: typeof Link2; color: string; tipKey: TranslationKey }[] = [
  { Icon: Link2, color: 'text-indigo-400', tipKey: 'pw.rank.matched.title' },
  { Icon: TrendingDown, color: 'text-rose-400', tipKey: 'pw.rank.beats.title' },
  { Icon: Sigma, color: 'text-sky-400', tipKey: 'pw.rank.avg.title' },
  { Icon: AlignCenterHorizontal, color: 'text-violet-400', tipKey: 'pw.rank.median.title' },
  { Icon: PackageX, color: 'text-amber-400', tipKey: 'pw.rank.stock.title' },
]
  ...
  return (
    <div className="bg-surface rounded-lg p-4 h-full">
      <div className="flex items-baseline justify-between mb-3 gap-2">
        <div className="text-sm font-semibold text-white">{t('pw.rank.title')}</div>
        {onOpenAudit && (
        ...
        <div className="text-[11px] text-white/35 ml-auto">{t('pw.rank.sort')}</div>
      </div>
      {rows.length === 0 ? (
        ...
        <table className="w-full text-xs tabular-nums">
          <thead className="sticky top-0 z-10 bg-surface">
            <tr className="text-[10px] uppercase tracking-wide text-right bg-well">
              <th className="text-left font-semibold py-2 pl-2 rounded-l-md text-white/60">{t('pw.col.competitor')}</th>
              {HEAD_COLS.map((c, i) => {
                const Icon = c.Icon
                return (
