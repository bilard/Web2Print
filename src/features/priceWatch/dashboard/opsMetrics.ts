...
interface OpsCompetitor {
  siteId: string
  domain: string
  indexed: number          // fiches indexées (dédupliquées)
  progress: number         // 0..1 balayage du cycle courant
  sweeps: number           // cycles complets du catalogue concurrent
  cumulMs: number          // temps de moisson cumulé (toutes passes)
...
function opsCompetitorOf(s: CompetitorStat): OpsCompetitor {
  const indexed = s.audit?.indexed ?? 0
  const h = s.harvest
  const sweeps = h?.sweeps ?? 0
  const cumulMs = h?.cumulMs ?? 0
  return {
    siteId: s.siteId, domain: s.domain, indexed,
    progress: Math.max(0, Math.min(1, h?.progress ?? 0)), sweeps, cumulMs,
    cycleMs: sweeps > 0 ? Math.round(cumulMs / sweeps) : null,
    pctPrice: s.audit?.pctPrice ?? 0,
  }
}

export function buildOpsCockpit(report: StoredReport): OpsCockpit {
  ...
  return {
    ...
    cyclesDone, slowestCycle, runAt: report.runAt,
