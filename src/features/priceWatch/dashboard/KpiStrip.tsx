...
// Bandeau de KPI dense du cockpit. Métriques FIABLES (kpis, non biaisées par le cap).
// Delta vs le point de tendance précédent. Chiffres en tabular-nums (alignement colonne).
...
function Tile({ label, value, sub, accent, delta }: {
  label: string; value: string; sub?: string; accent?: string; delta?: React.ReactNode
}) {
  return (
...
export function KpiStrip({ ck, history }: { ck: Cockpit; history: KpiHistoryPoint[] }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 2xl:grid-cols-8 gap-2">
      <Tile label="Tenue de prix" value={holdTxt} accent="text-emerald-400"
        sub="aligné ou + bas" />
      ...
      <Tile label="Impact unitaire" value={eur(ck.totalGapEur)} accent="text-rose-400"
        sub="Σ écart vs + bas" />
      ...
      <Tile label="Concurrents" value={String(ck.competitorsCount)}
        sub={`${k.comparisons} comparaisons`} />
      <Tile label="Ruptures conc." value={k.ruptures.toLocaleString('fr-FR')} accent="text-amber-400"
        sub="opportunités" />
