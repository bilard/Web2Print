...
export function HeatmapMatrix({ ck }: { ck: Cockpit }) {
      ...
      <div className="overflow-x-auto">
        <table className="border-separate border-spacing-0.5 text-xs tabular-nums">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.siteId}>
                <td className="sticky left-0 bg-surface pr-2 text-white/70 text-right whitespace-nowrap max-w-[150px] truncate" title={r.domain}>
                  {r.domain}
                </td>
                {cols.map((f) => {
                  ...
                  return (
                    <td
                      ...
                      className="h-8 min-w-[52px] text-center rounded-sm border border-white/5 text-white/85"
