...
export function GDrivePickerModal({ open, onClose, onPick, mimeFilter = 'all', foldersOnly = false, multiple = false, onPickMultiple, title }: Props) {
  ...
  return createPortal(
    ...
    >
      <div
        className="w-[860px] max-w-[95vw] h-[640px] max-h-[90vh] bg-background border border-white/10 rounded-xl shadow-2xl flex overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Sidebar */}
        <aside className="w-48 shrink-0 bg-background border-r border-white/[0.06] flex flex-col gap-3 p-3">
