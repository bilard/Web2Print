...
describe('réglage du seuil', () => {
  it('pose un seuil au-dessus par défaut', () => {
    const onApply = vi.fn()
    render(<BiAlertField tile={tile()} canEdit onApply={onApply} />)
  ...
  it('bascule le SENS sans perdre la valeur', () => {
    const onApply = vi.fn()
    render(<BiAlertField tile={tile({ op: 'gt', value: 30 })} canEdit onApply={onApply} />)
  ...
  it('reste inerte sans tuile sélectionnée ou sans droit d’édition', () => {
    const onApply = vi.fn()
    const { unmount } = render(<BiAlertField tile={null} canEdit onApply={onApply} />)
    expect((screen.getByRole('spinbutton') as HTMLInputElement).disabled).toBe(true)
    unmount()
    render(<BiAlertField tile={tile()} canEdit={false} onApply={onApply} />)
    expect((screen.getByRole('spinbutton') as HTMLInputElement).disabled).toBe(true)
  })
})
