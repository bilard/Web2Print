...
import { Users, Shield, ScrollText, BarChart3, Building2 } from 'lucide-react'
...
export function AccessAdminPage() {
  const [tab, setTab] = useState<'users' | 'roles' | 'companies' | 'audit' | 'analytics'>('users')
  useModuleIntent('access', (action) => {
    if (action === 'tab:users') setTab('users')
    else if (action === 'tab:roles') setTab('roles')
    else if (action === 'tab:companies') setTab('companies')
    ...
    <div className="h-full overflow-hidden bg-background p-6 flex flex-col">
      <div className="max-w-[1800px] w-full mx-auto flex flex-col gap-4 flex-1 min-h-0">
        ...
        <nav className="flex gap-1 bg-white/[0.02] border border-white/5 rounded-xl p-1 self-start">
          {([['users', t('ac.tab.users'), Users], ['roles', t('ac.tab.roles'), Shield], ['companies', t('ac.tab.companies'), Building2], ['audit', t('ac.tab.audit'), ScrollText], ['analytics', t('ac.tab.analytics'), BarChart3]] as const).map(([id, label, Icon]) => (
        ...
        <div className="flex-1 min-h-0 overflow-y-auto -mx-1 px-1">
          {tab === 'users' ? <UsersTab /> : tab === 'roles' ? <RolesTab /> : tab === 'companies' ? <CompaniesTab /> : tab === 'audit' ? <AuditTab /> : <AnalyticsTab />}
        </div>
      </div>
    </div>
  )
