    ...
    <div className="flex-1 overflow-auto">
      <table className="w-full border-collapse" style={{ tableLayout: 'fixed' }}>
        ...
        <thead>
          {/* Row 1: Column name */}
          <tr>
            <th className="sticky top-0 left-0 z-30 w-10 bg-[#2a2a2a] border-b-2 border-r border-white/[0.08] align-middle">
              <AddColumnMenu onAdd={(type, label) => handleAddColumn(type, label, 'start')} />
            </th>
            {visibleColumns.map((col, colIdx) => (
              <th
                key={col.key}
                className={`sticky top-0 z-20 bg-[#2a2a2a] border-b-2 border-r border-white/[0.08] text-left relative group/col transition-all ${
            ...
            <th className="sticky top-0 z-20 w-10 bg-[#2a2a2a] border-b-2 border-white/[0.08] align-middle">
              <AddColumnMenu onAdd={handleAddColumn} />
            </th>
            <th className="sticky top-0 right-0 z-30 w-10 bg-[#2a2a2a] border-b-2 border-l border-white/[0.08]" />
          </tr>

          {/* Row 2: Type + Stats — sticky below row 1 (top ~37px) */}
          <tr>
            <th className="sticky top-[37px] left-0 z-30 w-10 bg-[#262626] border-b border-r border-white/[0.06]" />
            {visibleColumns.map((col, vColIdx) => (
              <th
                key={col.key}
                className={`sticky top-[37px] z-20 bg-[#262626] border-b border-r border-white/[0.06] text-left ${dragColIdx !== null && dragColIdx === vColIdx ? 'opacity-30' : ''}`}
                style={{ width: getColWidth(col) }}
              >
            ...
            <th className="sticky top-[37px] z-20 w-10 bg-[#262626] border-b border-white/[0.06]" />
            <th className="sticky top-[37px] right-0 z-30 w-10 bg-[#262626] border-b border-l border-white/[0.06]" />
          </tr>
        </thead>
...
function DataRow({
  ...
  return (
    ...
    >
      <td
        className={`sticky left-0 z-10 relative px-1 py-[7px] border-b border-r border-white/[0.05] text-center align-middle ${rowIdx % 2 === 1 ? 'bg-[#2a2a2a]' : 'bg-[#242424]'} group-hover:bg-[#303030] cursor-grab active:cursor-grabbing`}
        draggable
        onDragStart={(e) => {
          e.dataTransfer.effectAllowed = 'move'
      ...
      <td
        className={`sticky right-0 z-10 px-1 py-1.5 border-b border-l border-white/[0.05] text-center ${rowIdx % 2 === 1 ? 'bg-[#2a2a2a]' : 'bg-[#242424]'} group-hover:bg-[#303030]`}
        onClick={(e) => e.stopPropagation()}
      >
        {canDelete && (
          <button
