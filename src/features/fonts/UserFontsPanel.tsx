// « {t('fonts.myFonts')} » : upload de fichiers .woff2/.woff/.ttf/.otf + ajout d'une
...
export function UserFontsPanel() {
  ...
  const onFiles = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      try {
        await upload(file)
        toast.success(t('fonts.loaded', { name: file.name }))
      } catch (e) {
  ...
  const onAddGoogle = async () => {
    ...
    try {
      ...
      toast.success(t('fonts.googleAdded', { family }))
      setGInput('')
    } catch (e) {
      toast.error(String((e as Error).message))
    }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t('fonts.myFonts')}</span>
        <button type="button" disabled={busy} onClick={() => inputRef.current?.click()}
          ...
          {busy ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Upload className="w-3.5 h-3.5" />} {t('fonts.upload')}
        </button>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button type="button" disabled={busy || !gInput.trim()} onClick={() => void onAddGoogle()}
          ...
          <Plus className="w-3.5 h-3.5" /> {t('fonts.add')}
        </button>
      </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {fonts.map((f) => (
            <span key={f.id} className="flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 rounded-md bg-surface-2 text-xs text-white">
              ...
              <button type="button" onClick={() => void remove(f).then(() => toast.success(t('fonts.deleted')))}
                className="text-muted-foreground hover:text-red-400" title={t('fonts.delete')}>
                <X className="w-3 h-3" />
              </button>
            </span>
          ))}
