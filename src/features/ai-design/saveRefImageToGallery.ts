...
/** Convertit un data URI (`data:image/png;base64,...`) en Blob. */
...
/** Charge un Blob image et retourne ses dimensions naturelles. */
...
/** Rasterise un blob image vers un JPEG de taille maxSize (thumbnail). */
...
export interface SaveRefImageArgs {
  dataUri: string
  projectId: string
  /** Nom lisible (ex: "Makita DUH752Z — ref Nano Banana"). */
  name: string
  /** Tags additionnels (la tag "nano-banana" + "design-ref" est toujours ajoutée). */
  tags?: string[]
}

/**
 * Upload direct vers Firebase Storage + Firestore, sans passer par le hook
 * useImageGallery (qui est React-only). Fire-and-forget depuis la pipeline.
 */
