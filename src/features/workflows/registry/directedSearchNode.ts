...
const directedSearchNode: NodeSpec<DirectedConfig, DirectedInputs, DirectedOutputs> = {
  ...
  configSchema: [
    ...
    { name: 'genericSites', kind: 'textarea', labelKey: 'node.directed-search.genericSites.label', help: 'Amazon, Cdiscount, Kramp… : recherche web par réf + Firecrawl (rendu JS + anti-bot). Nécessite une clé Firecrawl. Coût par réf — commence par 1 site pour valider.' },
    ...
    { name: 'descriptionColumn', kind: 'text', labelKey: 'node.directed-search.descriptionColumn.label', help: 'Ex : TEXT_VENTE_FR. DÉCISIF sur un catalogue de pièces adaptables : si ta référence article est un code INTERNE, aucun concurrent ne la porte — seules les réf. d’origine citées ici (« Origine: … ») permettent de trouver le produit.' },
    { name: 'productBudget', kind: 'number', labelKey: 'node.directed-search.productBudget.label', help: 'Nombre de produits testés par exécution. Chacun est cherché sur tous les sites. Les produits sont désormais traités 3 de front : ce budget tient dans le même temps qu’un tiers de sa valeur auparavant. ⚠ C’est LE plafond du débit — à 20/run, un catalogue de 75 000 réf. demande ~29 jours pour un seul tour.' },
    {
      name: 'watchId', kind: 'text', labelKey: 'node.directed-search.watchId.label', help: 'Laisse VIDE : le suivi est celui du workflow (partagé avec « Comparer catalogue » du même workflow — les prix trouvés remontent alors dans le dashboard).',
      disabledWhen: (_c, wired) => wired('sites'),
