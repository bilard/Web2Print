...
import { extractCalls, frenchLiterals } from '@/lib/i18n/frenchLiterals'
...
/**
 * Ce qui n'est PAS de la prose : un niveau de log, une clé du catalogue.
 * Le scanner de littéraux est partagé avec `userMessages.test.ts` — deux copies
 * dériveraient, et `npm run dup` les refuserait.
 */
function offendingLiterals(code: string): string[] {
  return frenchLiterals(code, (lit) => LEVELS.has(lit) || lit.startsWith('run.'))
}
  ...
  for (const { type, files } of DONE) {
    for (const file of files) {
      it(`${type} — ${file} ne logue aucun texte en dur`, () => {
        ...
        for (const call of extractCalls(source, LOG_OPENER)) {
