...
export function PreflightBanner({ issues, errors, onOpen }: {
  ...
  return (
    <button
      onClick={onOpen}
      title="Voir le détail et sauter à la carte concernée"
    ...
    >
      ...
      <span className="font-medium">
        {blocking ? plural(errors, 'incohérence') : plural(warnings, 'avertissement')}
      </span>
      <span className="text-white/50 truncate">
        {blocking && warnings > 0 ? `+ ${plural(warnings, 'avertissement')} · ` : ''}
        {issues[0].nodeLabel} — {issues[0].message}
      </span>
      <span className="ml-auto shrink-0 underline underline-offset-2 opacity-80">Voir</span>
    </button>
  )
}
