...
export function PromptToFlowModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div
        className="w-full max-w-lg rounded-xl border border-neutral-800 bg-surface p-4 space-y-3"
            ...
            <div className="flex flex-wrap gap-1.5">
              {EXAMPLES.map((ex) => (
                <button
                  ...
                  className="text-[11px] text-left rounded border border-neutral-700 bg-background px-2 py-1 text-white/50 hover:text-white/80 hover:border-neutral-600"
                >
                  {ex}
                </button>
              ))}
