...
import { X, Search, Layers, Check, Loader2 } from 'lucide-react'
...
export function EditorTaxonomyPicker({
  open,
  projectId,
  onClose,
  ...
  // Construit la liste des feuilles avec leur chemin
  const allLeaves = useMemo<LeafEntry[]>(() => {
    if (!taxonomies) return []
    const entries: LeafEntry[] = []
    for (const tax of taxonomies) {
      const nodes = tax.nodes
      // Identifie les feuilles
  ...
  // Trouve le nœud actuellement lié au projet (s'il existe)
    ...
    for (const tax of taxonomies) {
      for (const node of Object.values(tax.nodes)) {
        if (node.linkedProjectIds.includes(projectId)) {
          return { taxonomyId: tax.id, nodeId: node.id }
        }
      }
    }
    return null
  ...
  const visible = useMemo(() => {
    ...
    return allLeaves
      .filter((e) =>
        taxonomyFilter === 'all' ? true : e.taxonomy.id === taxonomyFilter
      )
      .filter((e) =>
        q.length === 0 ? true : e.pathString.toLowerCase().includes(q)
      )
      .sort((a, b) => a.pathString.localeCompare(b.pathString))
  }, [allLeaves, taxonomyFilter, search])

  if (!open || !projectId) return null

  const isCurrent = (taxonomyId: string, nodeId: string) =>
    currentLink?.taxonomyId === taxonomyId && currentLink?.nodeId === nodeId

  const handlePick = async (entry: LeafEntry) => {
    if (isCurrent(entry.taxonomy.id, entry.node.id)) {
      // Déjà lié → délier
      unlink.mutate(
        {
          taxonomyId: entry.taxonomy.id,
    ...
    // Délier l'ancien si présent, puis lier le nouveau
    if (currentLink) {
      unlink.mutate({
        taxonomyId: currentLink.taxonomyId,
      ...
      >
        ...
        <div className="px-5 py-2 mt-2 border-y border-white/[0.06]">
          <span className="text-[11px] text-white/40">
            {visible.length} nœud{visible.length !== 1 ? 's' : ''} disponible
            {visible.length !== 1 ? 's' : ''}
          </span>
        </div>
                ...
                return (
                  <li key={`${entry.taxonomy.id}:${entry.node.id}`}>
                    <button
                      ...
                      disabled={isPending}
                      className={`w-full flex items-center gap-3 p-2 rounded-lg transition-colors text-left group disabled:opacity-50 disabled:cursor-not-allowed ${
                        current
                          ? 'bg-indigo-500/[0.12] hover:bg-indigo-500/[0.18]'
                          : 'hover:bg-white/[0.04]'
                      }`}
                    >
                      <span
