    ...
    <div className="flex flex-col gap-1.5">
      ...
      {existingFiles.length > 0 && (
        <div className="flex flex-col gap-1 mb-1">
          {existingFiles.map((f, idx) => (
            <div
              key={`${f.url}_${idx}`}
              className="flex items-center gap-3 bg-background border border-white/[0.08] rounded-md px-3 py-1.5"
            >
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 bg-background border border-dashed border-white/[0.12] hover:border-indigo-500/60 rounded-md px-3 py-3 text-[12px] text-white/60 hover:text-white disabled:opacity-50"
        >
