    ...
    <div className="flex flex-col h-full">
            ...
            {slideCount > 0 ? (
              <ul className="grid grid-cols-2 lg:grid-cols-3 gap-2">
                {brief.deck?.slides.map((s, i) => (
                  <li
                    key={i}
                    className="text-[11px] text-white/70 bg-surface-2 border border-white/[0.06] rounded px-3 py-2 truncate"
                    title={s.title}
                  >
      ...
      <div className="border-t border-white/[0.06] bg-surface-2 px-6 py-3 flex justify-end shrink-0">
        <button
          ...
          className="flex items-center gap-1.5 text-[12px] text-[#fff] bg-indigo-500 hover:bg-indigo-600 px-4 py-2 rounded-md disabled:opacity-50"
        >
          Étape suivante
