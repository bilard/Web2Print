      ...
      {isSelect && (
        <>
          ...
          <textarea
            ...
            rows={5}
            className="bg-background border border-white/[0.08] rounded-md px-3 py-2 text-[13px] text-white focus:outline-none focus:border-indigo-500/60 font-mono"
          />
        </>
      )}
...
function Input({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <input
      ...
      className="bg-background border border-white/[0.08] rounded-md px-3 py-2 text-[13px] text-white focus:outline-none focus:border-indigo-500/60"
    />
  )
}
