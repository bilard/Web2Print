...
import { useAddNode, useDeleteNode, usePurgeEmptyNodes } from '@/features/taxonomy/useTaxonomyMutations'
import { getAllDescendantIds } from '@/features/taxonomy/taxonomyUtils'
...
export default function TaxonomiesPage({ embedded = false }: TaxonomiesPageProps) {
  ...
  const { selectedTaxonomyId, selectedNodeId, setSelectedNode, expandAll } = useTaxonomyStore()
  const addNode = useAddNode()
  const deleteNode = useDeleteNode()
  const purgeEmptyNodes = usePurgeEmptyNodes()
  ...
  const handleAddNode = () => {
    ...
    if (selectedNodeId) expandAll([selectedNodeId])
  }

  const handleDeleteSelected = () => {
    if (!selectedTaxonomyId || !selectedNodeId) return
    deleteNode.mutate({ taxonomyId: selectedTaxonomyId, nodeId: selectedNodeId })
  ...
  const handlePurge = () => {
    if (!selectedTaxonomyId) return
    purgeEmptyNodes.mutate({ taxonomyId: selectedTaxonomyId })
  }

  const selectedNodeLabel = selectedNodeId && selectedTaxonomy
    ? selectedTaxonomy.nodes[selectedNodeId]?.label ?? null
    : null

  const selectedNodeDescendantCount = selectedNodeId && selectedTaxonomy
    ? getAllDescendantIds(selectedTaxonomy.nodes, selectedNodeId).length
    : 0

  // Compte les nœuds "Nouveau nœud" sans enfants et sans projets
  const emptyNodeCount = selectedTaxonomy
    ? (() => {
        const childCounts = new Map<string, number>()
        for (const n of Object.values(selectedTaxonomy.nodes)) {
          if (n.parentId) childCounts.set(n.parentId, (childCounts.get(n.parentId) ?? 0) + 1)
        }
        return Object.values(selectedTaxonomy.nodes).filter(
          (n) => n.label === 'Nouveau nœud' && (childCounts.get(n.id) ?? 0) === 0 && n.linkedProjectIds.length === 0
        ).length
      })()
    : 0
            ...
            <>
              <div className="h-11 bg-[#161616] border-b border-white/[0.06] flex items-center px-4 gap-3 shrink-0">
                ...
                <button
                  ...
                  aria-label={selectedNodeId ? `Ajouter un enfant à ${selectedNodeLabel}` : 'Ajouter un nœud racine'}
                >
                  <Plus className="w-3.5 h-3.5" />
                  {selectedNodeId ? 'Ajouter nœud' : 'Nœud racine'}
                </button>
                <button
                  onClick={() => selectedNodeId ? setShowDeleteConfirm(true) : null}
                  ...
                  aria-label="Supprimer le nœud sélectionné"
                >
                ...
                {emptyNodeCount > 0 && (
                  <button
                    onClick={handlePurge}
                    className="flex items-center gap-1.5 text-[12px] text-red-400/60 hover:text-red-400 hover:bg-red-500/10 px-3 py-1.5 rounded-md transition-colors"
                    aria-label="Supprimer les nœuds vides"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    Purger ({emptyNodeCount})
                  </button>
                )}
              </div>
              <TaxonomyBreadcrumb taxonomy={selectedTaxonomy} />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Cette action supprimera « {selectedNodeLabel} »
              {selectedNodeDescendantCount > 0
                ? ` et ses ${selectedNodeDescendantCount} descendant(s)`
                : ''}
