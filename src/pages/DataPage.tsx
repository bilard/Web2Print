...
import {
  ...
  Loader2, Trash2, FolderOpen, Columns3, RefreshCw, FolderTree, Group, List, Globe,
...
type RightTab = 'fields' | 'taxonomy' | 'files'
    ...
    <div className="space-y-3">
      ...
      <div>
        ...
        <div className="space-y-px">
          ...
          <button onClick={() => handleToggleRightTab('files')} className={sidebarBtn(showRight && rightTab === 'files')}>
            <FolderOpen className="w-4 h-4 opacity-50" aria-hidden="true" />
            Fichiers
            {savedFiles.length > 0 && <span className="ml-auto text-[9px] text-white/30">{savedFiles.length}</span>}
            {showRight && rightTab === 'files' && <span className="ml-1 w-1.5 h-1.5 rounded-full bg-indigo-400/80" />}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className={`${embedded ? 'h-full' : 'h-screen'} bg-[#0f0f0f] text-white flex flex-col overflow-hidden`}>
      ...
      <header className="h-11 bg-[#161616] border-b border-white/[0.06] flex items-center px-3 gap-2 shrink-0">
        ...
        <div className="flex items-center gap-0.5">
          <button onClick={() => setImportModalOpen(true)} className={headerBtn}>
            <Upload className="w-3.5 h-3.5" />
            Importer
          </button>
          <button onClick={() => setScrapingOpen(true)} className={headerBtn}>
      ...
      <div className="flex-1 flex overflow-hidden">
        {hasData ? (
          ...
          <div className="flex-1 flex">
              ...
              <div className="flex flex-col items-center gap-6 max-w-sm text-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-white/40">
                    Importez un fichier Excel ou CSV, ou creez un nouveau tableau vide.
                  </p>
                </div>
                <div className="flex gap-3 flex-wrap justify-center">
                  <button
                    onClick={() => setImportModalOpen(true)}
                    className="flex items-center gap-2 bg-indigo-500 hover:bg-indigo-600 text-white text-sm font-medium px-5 py-2.5 rounded-xl transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white/60 text-sm font-medium px-5 py-2.5 rounded-xl transition-colors"
                  >
            ...
            {/* Saved files sidebar in empty state */}
            {(showRight && rightTab === 'files') || savedFiles.length > 0 ? (
              <div className="w-72 bg-[#161616] border-l border-white/10 flex flex-col shrink-0 overflow-hidden">
