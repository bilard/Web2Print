/**
 * Enregistre reel.html (?clean) en vidéo MP4 1080×1920.
 * Usage : node record-video.mjs
 * Sortie : marketing/promo-reel/video/promo-reel.mp4
 */
...
await page.goto(`http://localhost:${port}/reel.html?clean`, { waitUntil: 'networkidle' });
...
const webm = join(outDir, 'promo-reel.webm');
await rename(await video.path(), webm);
console.log('Conversion MP4…');
const mp4 = join(outDir, 'promo-reel.mp4');
