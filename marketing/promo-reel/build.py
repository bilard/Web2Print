...
.rl-stage{flex:1 1 auto;min-height:0;display:flex;align-items:center;justify-content:center;margin-top:30px;overflow:visible}
.rl-stage .scene-visual{position:relative;width:600px;transform-origin:center center}
.rl-stage .screen{width:600px}
...
(function(){
  ...
  function fitMock(c){
    ...
    var w=sv.offsetWidth||600, h=sv.offsetHeight||400;
    var availW=st.clientWidth, availH=st.clientHeight;
    var s=Math.min(availW/w, availH/h);
    s=Math.max(1, Math.min(s, 2.2));               // agrandit le mock-up jusqu'à remplir le cadre
