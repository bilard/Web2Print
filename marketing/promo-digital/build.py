...
OUT = HERE / "digital.html"
PUB = ROOT / "public/digital/index.html"   # copie servie sur la route /digital
...
(function(){
  ...
  function fitMock(c){
    ...
    if(sH>s) s=Math.min(sH, sW*1.08);
    s=Math.max(1, Math.min(s, 2.9));
    sv.style.transform="scale("+s.toFixed(3)+")";
  }
  function show(n){
    if(timer)clearTimeout(timer);
    cards.forEach(function(c,k){c.classList.toggle("on",k===n);});
    retrigger(cards[n]); i=n;
    requestAnimationFrame(function(){requestAnimationFrame(function(){fitMock(cards[n]);});});
    cnt.textContent=(n+1)+" / "+cards.length;
    segs.forEach(function(s,k){
      s.classList.toggle("done",k<i); s.classList.toggle("cur",k===i);
      var bar=s.querySelector("i"); bar.style.transition="none"; bar.style.width=(k<i?"100%":"0");
    });
    if(playing){
      var bar=segs[i].querySelector("i");
      requestAnimationFrame(function(){requestAnimationFrame(function(){
        bar.style.transition="width "+DUR[i]+"ms linear"; bar.style.width="100%";});});
...
# Synchronise la copie servie sur /digital (évite la dérive avec la source)
PUB.parent.mkdir(parents=True, exist_ok=True)
PUB.write_text(doc, encoding="utf-8")
shutil.copytree(HERE / "assets", PUB.parent / "assets", dirs_exist_ok=True)
print("Synchronisé :", PUB)
