// functions/src/priceWatch/catalog/categoryTargeting.ts
// Jumeau SERVEUR du module pur client (garder les deux fichiers IDENTIQUES).
...
// (Le schéma de sortie structuré TARGETING_SCHEMA_FOR_LLM n'existe que côté client :
//  `generateJson` en a besoin, alors que `callLlm` serveur rend du texte brut passé à
//  `applyTargeting`. Seule divergence assumée entre les deux jumeaux.)
