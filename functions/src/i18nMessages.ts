...
const MESSAGES = {
  ...
  'run.compareCatalog.siteIndexCount': {
    fr: "{domain} : {count} produit(s) dans l'index — {pctRef} % portent une référence, {pctPrice} % un prix, {pctImage} % un visuel.",
    en: '{domain}: {count} product(s) in the index — {pctRef}% carry a reference, {pctPrice}% a price, {pctImage}% an image.',
  },
  ...
  'run.compareCatalog.emptySites': {
    fr: '{count} concurrent(s) sans aucun appariement, écartés de la feuille : {sites}.',
    en: '{count} competitor(s) with no match at all, left out of the sheet: {sites}.',
  },
